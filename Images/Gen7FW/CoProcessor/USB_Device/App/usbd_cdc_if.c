...
static uint8_t uart_rx_byte, uart_tx_byte;
static volatile uint8_t uart_tx_active, usb_tx_busy[CDC_PORT_COUNT];
...
static int8_t cdc_init(uint8_t p){USBD_CDC_SetTxBuffer(&hUsbDeviceFS,p,UserTxBufferFS[p],0);USBD_CDC_SetRxBuffer(&hUsbDeviceFS,p,UserRxBufferFS[p]);if(p==CDC_PORT_BRIDGE){bridge_out_armed=1;HAL_NVIC_SetPriority(USART3_IRQn,5,0);HAL_NVIC_EnableIRQ(USART3_IRQn);HAL_UART_Receive_IT(&huart3,&uart_rx_byte,1);}return USBD_OK;}
...
static int8_t cdc_receive(uint8_t p,uint8_t*b,uint32_t*n){
 if(p==CDC_PORT_BRIDGE){bridge_out_armed=0;for(uint32_t i=0;i<*n;i++){uint16_t x=(uart_tx_head+1U)%CDC_BRIDGE_BUF_SIZE;if(x==uart_tx_tail)break;uart_tx_ring[uart_tx_head]=b[i];uart_tx_head=x;}uart_tx_start();bridge_out_arm_if_ready();return USBD_OK;}
...
static void uart_tx_start(void){if(uart_tx_active||uart_tx_head==uart_tx_tail)return;uart_tx_active=1;uart_tx_byte=uart_tx_ring[uart_tx_tail];HAL_UART_Transmit_IT(&huart3,&uart_tx_byte,1);}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef*u){if(u->Instance==USART3){uint16_t x=(uart_rx_head+1U)%CDC_BRIDGE_BUF_SIZE;if(x!=uart_rx_tail){uart_rx_ring[uart_rx_head]=uart_rx_byte;uart_rx_head=x;}HAL_UART_Receive_IT(&huart3,&uart_rx_byte,1);}}
void HAL_UART_TxCpltCallback(UART_HandleTypeDef*u){if(u->Instance==USART3){uart_tx_tail=(uart_tx_tail+1U)%CDC_BRIDGE_BUF_SIZE;if(uart_tx_head!=uart_tx_tail){uart_tx_byte=uart_tx_ring[uart_tx_tail];HAL_UART_Transmit_IT(&huart3,&uart_tx_byte,1);}else uart_tx_active=0;}}
...
void CDC_Bridge_Process(void){bridge_out_arm_if_ready();uint8_t a=requested_action;if(a){requested_action=0;if(a==ACTION_BOOTLOADER){reset_main(GPIO_PIN_SET);send_control("BOOTLOADER OK\r\n");}else if(a==ACTION_APP){reset_main(GPIO_PIN_RESET);send_control("APP OK\r\n");}else if(a==ACTION_RESET){reset_main(GPIO_PIN_RESET);send_control("RESET OK\r\n");}else{char s[64];snprintf(s,sizeof(s),"STATUS BOOTSEL=%u RESET=%u\r\n",HAL_GPIO_ReadPin(BOOTSEL_MAIN_MCU_GPIO_Port,BOOTSEL_MAIN_MCU_Pin),HAL_GPIO_ReadPin(RESET_MAIN_MCU_GPIO_Port,RESET_MAIN_MCU_Pin));send_control(s);}}
...
uint8_t CDC_IsBridgeMode(void){return 1;}void CDC_DebugReportStartup(void){reset_main(GPIO_PIN_RESET);}
