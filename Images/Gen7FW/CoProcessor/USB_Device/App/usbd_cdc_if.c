...
static void uart_tx_start(void), uart_flush(void), bridge_out_arm_if_ready(void), reset_main(GPIO_PinState), send_control(const char *);
...
static void uart_tx_start(void){if(uart_tx_head!=uart_tx_tail)USART3->CR1|=USART_CR1_TXEIE_TXFNFIE;}
static void uart_flush(void){HAL_NVIC_DisableIRQ(USART3_IRQn);USART3->CR1&=~USART_CR1_TXEIE_TXFNFIE;uart_rx_head=uart_rx_tail=0;uart_tx_head=uart_tx_tail=0;USART3->RQR=USART_RQR_RXFRQ;USART3->ICR=USART_ICR_PECF|USART_ICR_FECF|USART_ICR_NECF|USART_ICR_ORECF;HAL_NVIC_EnableIRQ(USART3_IRQn);}
...
void CDC_Bridge_Process(void){bridge_out_arm_if_ready();uint8_t a=requested_action;if(a){requested_action=0;if(a==ACTION_BOOTLOADER){uart_flush();reset_main(GPIO_PIN_SET);send_control("BOOTLOADER OK\r\n");}else if(a==ACTION_APP){uart_flush();reset_main(GPIO_PIN_RESET);send_control("APP OK\r\n");}else if(a==ACTION_RESET){uart_flush();reset_main(GPIO_PIN_RESET);send_control("RESET OK\r\n");}else{char s[128];snprintf(s,sizeof(s),"STATUS BOOTSEL=%u RESET=%u UART_ERRORS=%lu RX_DROPPED=%lu TX_DROPPED=%lu\r\n",HAL_GPIO_ReadPin(BOOTSEL_MAIN_MCU_GPIO_Port,BOOTSEL_MAIN_MCU_Pin),HAL_GPIO_ReadPin(RESET_MAIN_MCU_GPIO_Port,RESET_MAIN_MCU_Pin),(unsigned long)uart_errors,(unsigned long)uart_rx_dropped,(unsigned long)uart_tx_dropped);send_control(s);}}
