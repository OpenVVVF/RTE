...
bool GenerateEmptyDomainFiles(const std::filesystem::path& generatedDir,
    ...
    source << "};\n";
    source << "const size_t g_" << domainCpp << "_config_count = 0;\n\n";
    source << "const RteParamDesc g_" << domainCpp << "_vars[] = {\n";
    source << "    {nullptr, nullptr, nullptr},\n";
    source << "};\n";
    source << "const size_t g_" << domainCpp << "_var_count = 0;\n\n";
    source << "} // namespace app\n";
...
bool WriteBuildInfo(const std::filesystem::path& generatedDir,
                    const NodeAPI::Graph& graph, const std::string& graphJson,
                    std::string& error) {
    std::uint64_t hash = 14695981039346656037ULL;
    for (const unsigned char byte : graphJson) {
        hash ^= byte;
    ...
    nlohmann::json manifest = {{"schema", 1}, {"graph", graph.GetName()},
                               {"graph_hash", hashText.str()},
                               {"nodes", nlohmann::json::array()},
                               {"signals", nlohmann::json::array()}};
    for (const auto& node : graph.GetNodes()) {
        manifest["nodes"].push_back({{"id", node.id}, {"type", node.type},
                                      {"domain", node.domain}});
        if (node.type != "Debug.TelemetryLog") continue;
        const auto key = node.parameters.find("Key");
        if (key == node.parameters.end() || key->second.empty()) continue;
        nlohmann::json signal = {{"name", key->second}, {"logger_node", node.id},
                                 {"domain", node.domain}};
        for (const auto& connection : graph.GetConnections()) {
            if (connection.to.nodeId != node.id || connection.to.portName != "Value") continue;
            signal["source_node"] = connection.from.nodeId;
            signal["source_port"] = connection.from.portName;
            if (const auto port = graph.FindPort(connection.from)) {
                signal["unit"] = std::string(NodeAPI::GetUnitLabel(port->type.quantity));
                if (!port->description.empty()) signal["description"] = port->description;
            }
            break;
        }
        manifest["signals"].push_back(std::move(signal));
    }
    const std::string manifestText = manifest.dump(-1, ' ', true);
    std::ostringstream header;
    header << "#pragma once\n// Generated by RTECodeEmitter from the effective graph.\n";
    header << "#define RTE_GRAPH_NAME " << CppStringLiteral(graph.GetName()) << "\n";
    header << "#define RTE_GRAPH_HASH " << CppStringLiteral(hashText.str()) << "\n";
    header << "#define RTE_BUILD_NODE_COUNT " << graph.GetNodes().size() << "\n";
    header << "#define RTE_BUILD_MANIFEST " << CppStringLiteral(manifestText) << "\n";
    return WriteFileText(generatedDir / "rte_build_info.h", header.str(), error);
}
...
bool Emitter::Run(const EmitterOptions& options) const {
    ...
    if (!options.dryRun) {
        ...
        if (!WriteBuildInfo(generatedDir, graph, jsonText, genError)) {
            logger_.Error("Build identity generation failed: " + genError);
            return false;
        }
