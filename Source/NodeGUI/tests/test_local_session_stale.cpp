...
TEST(RteStudioSession, StoppedSignalIsNotAZeroAndHistoryWindowExpires) {
    ...
    EXPECT_TRUE((*stopped)["signals"]["isr_current"].is_null());
    EXPECT_EQ((*stopped)["signal_status"]["isr_current"]["state"], "control_stopped");
    ...
    EXPECT_TRUE((*stoppedSnapshot)["values"]["isr_current"]["value"].is_null());
    EXPECT_EQ((*stoppedSnapshot)["values"]["isr_current"]["state"], "control_stopped");
