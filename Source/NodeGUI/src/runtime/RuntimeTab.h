...
// Top-level widget of the "Runtime" screen: link-status header, graph-layout
// preset row, and the Telemetry panel. Firmware Update is a separate top-level
// screen (see MainWindow). The signal table and console live in dock widgets
// on the main window (see GetSignalTable / GetConsole).
class RuntimeTab : public QWidget {
    Q_OBJECT

public:
    RuntimeTab(RuntimeController* controller, QWidget* parent = nullptr);
